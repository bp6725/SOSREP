...
from SOSREP.third_party.ADbench.myutils import Utils
...
class DataGenerator():
    ...
    def generator(self, X=None, y=None, minmax=True,
                  ...
                  noise_type=None, duplicate_times:int=2, contam_ratio=1.00, noise_ratio:float=0.05, return_all_data=False):
        '''
        ...
        if self.dataset is None:
            ...
            if return_all_data :
                ...
                def pick_samples(X, y, N=5000, positive_ratio=0.15):
                    """
                    Pick N samples from X and y where a specific ratio is positive.
                    If X has less than N samples, return the entire X and y.

                    :param X: Data samples
                    :param y: Corresponding labels
                    :param N: Total number of samples to pick
                    :param positive_ratio: Ratio of positive samples to pick
                    :return: Selected X and y
                    """
                    total_samples = len(X)

                    # If X has less than N samples, return the entire datasets
                    if total_samples <= N:
                        return X, y

                    # Calculate the number of positive samples needed
                    num_positives_needed = int(N * positive_ratio)
                    ...
                    if len(positive_indices) < num_positives_needed:
                        num_positives_needed = len(positive_indices)

                    # Number of negatives needed to fill up to N
                    num_negatives_needed = N - num_positives_needed

                    # Randomly pick positive and negative samples
                    chosen_positives = np.random.choice(positive_indices, num_positives_needed, replace=False)
                    chosen_negatives = np.random.choice(negative_indices, num_negatives_needed, replace=False)

                    # Combine and shuffle the chosen indices
                    chosen_indices = np.concatenate([chosen_positives, chosen_negatives])
                    np.random.shuffle(chosen_indices)

                    # Select the samples and labels
                    X_selected = X[chosen_indices]
                    y_selected = y[chosen_indices]
                ...
                X_train, y_train = pick_samples(X_train, y_train)
